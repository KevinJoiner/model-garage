...
// ToLowVoltageBatteryCurrentVoltage0 converts data from field 'signals.30' of type string to 'Vehicle.LowVoltageBattery.CurrentVoltage' of type float64.
...
func ToPowertrainRange0(originalDoc []byte, val string) (float64, error) {
	...
	rangeKm, err := strconv.ParseFloat(val, 64)
	if err != nil {
		return 0, fmt.Errorf("could not parse float: %v", err)
	}
	return rangeKm * 1000, nil
}
