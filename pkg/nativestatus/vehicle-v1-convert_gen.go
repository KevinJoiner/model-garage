...
func SignalsFromV1Data(baseSignal vss.Signal, jsonData []byte) ([]vss.Signal, []error) {
	...
	val, err = PowertrainTractionBatteryChargingAddedEnergyFromV1Data(jsonData)
	if err != nil {
		if !errors.Is(err, errNotFound) {
			errs = append(errs, fmt.Errorf("failed to get 'PowertrainTractionBatteryChargingAddedEnergy': %w", err))
		}
	} else {
		sig := vss.Signal{
...
// PowertrainTractionBatteryChargingAddedEnergyFromV1Data converts the given JSON data to a float64.
func PowertrainTractionBatteryChargingAddedEnergyFromV1Data(jsonData []byte) (ret float64, err error) {
	var errs error
	var result gjson.Result
	result = gjson.GetBytes(jsonData, "data.chargeEnergyAdded")
	if result.Exists() && result.Value() != nil {
		val, ok := result.Value().(float64)
		if ok {
			...
			errs = errors.Join(errs, fmt.Errorf("failed to convert 'data.chargeEnergyAdded': %w", err))
		} else {
			errs = errors.Join(errs, fmt.Errorf("%w, field 'data.chargeEnergyAdded' is not of type 'float64' got '%v' of type '%T'", convert.InvalidTypeError(), result.Value(), result.Value()))
		}
	}

	if errs == nil {
		return ret, fmt.Errorf("%w 'PowertrainTractionBatteryChargingAddedEnergy'", errNotFound)
	}
